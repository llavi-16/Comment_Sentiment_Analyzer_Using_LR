...
API_KEY = os.getenv("YOUTUBE_API_KEY")
if not API_KEY:
    raise ValueError("YouTube API Key is missing. Please set it as an environment variable.")
...
try:
    sentiment_pipeline = load(MODEL_FILE)
    logging.info("Successfully loaded pre-trained sentiment model.")
except FileNotFoundError:
    logging.error(f"Model file not found at {MODEL_FILE}. Make sure the model is trained during the build step.")
    # The app can't run without the model, so we don't assign it.
except Exception as e:
...
async def analyze_video_comments(request: VideoRequest):
    if not sentiment_pipeline:
        raise HTTPException(status_code=503, detail="Sentiment model is not available. The application failed to load it on startup.")
    ...
    NEUTRAL_LOWER_BOUND = 0.4
    NEUTRAL_UPPER_BOUND = 0.6

    try:
        predictions_proba = sentiment_pipeline.predict_proba(comments)
        for proba in predictions_proba:
            positive_probability = proba[1]
            if positive_probability > NEUTRAL_UPPER_BOUND:
                sentiment_counts["positive"] += 1
            elif positive_probability < NEUTRAL_LOWER_BOUND:
                sentiment_counts["negative"] += 1
            else:
